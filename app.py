...
def main():
    if uploaded_file:
        try:
            ...
            # ✅ Keep AIAnalyzer instance in session state
            ...
            if st.button("Generate Insights"):
                with st.spinner("Generating insights..."):
                    ...
                    insights = analyzer.generate_insights(data_summary)
                if insights:
            ...
            # 🚀 Example placeholders for other AI methods
            if st.button("Generate Narrative Report"):
                report = analyzer.generate_narrative_report(
                    analysis=stats,
                    viz_metadata=[],   # fill this with your visualizations metadata
            ...
            if st.button("Analyze Patterns"):
                pattern_result = analyzer.analyze_patterns(df.to_dict())
                if pattern_result:
                    st.write(pattern_result)

            if st.button("Suggest Next Steps"):
                next_steps = analyzer.suggest_next_steps(stats)
                if next_steps:
                    st.write(next_steps)
